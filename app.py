    ...
    if st.button("🚀 Analyze Wind Data"):
        with st.spinner("Fetching wind data and performing analysis..."):
            ...
            with tab4:
                ...
                # Get current time (June 3rd in your example)
                ...
                # Prepare historical data - show data from current day and previous day
                ...
                # Historical data (up to current time)
                fig.add_trace(go.Scatter(
                ...
                # Prediction line (starting from current time)
                fig.add_trace(go.Scatter(
                    x=pred_df['Time'],
                    y=pred_df['Predicted Wind Speed (m/s)'],
                    name='Forecast',
                ...
                # Add vertical line to mark current time
                fig.add_vline(
                    x=current_time,
                    line_dash="dash",
                    line_color="white",
                ...
                for day in [current_time - timedelta(days=1), current_time, current_time + timedelta(days=1)]:
                    fig.add_vrect(
                        x0=day.replace(hour=0, minute=0, second=0),
                        x1=day.replace(hour=6, minute=0, second=0),
                        fillcolor="rgba(0,0,0,0.2)",
                        layer="below",
                        line_width=0
                    )
                    fig.add_vrect(
                        x0=day.replace(hour=18, minute=0, second=0),
                        x1=day.replace(hour=0, minute=0, second=0) + timedelta(days=1),
                        fillcolor="rgba(0,0,0,0.2)",
                        layer="below",
                        line_width=0
                    )
                ...
                fig.update_layout(
                    title=f"Wind Speed Forecast (Current: {current_time.strftime('%b %d')})",
                    xaxis_title="Time",
                    yaxis_title="Wind Speed (m/s)",
                    template="plotly_dark",
                    hovermode="x unified",
                    legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
                    xaxis_range=[start_time, pred_df['Time'].iloc[-1]]
                )
                st.plotly_chart(fig, use_container_width=True)
